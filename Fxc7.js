	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} SYMPHONY MISS YOU:D`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
        ...
        break
         ...
         case 'lirik':
             ...
             client.updatePresence(from, Presence.composing) 
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./Fxc7/loli.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, loli, image, {quoted: mek, caption: '\`\`\`loli\`\`\`'})
				await limitAdd(sender) 
				break
            case '!loli2': 
             client.updatePresence(from, Presence.composing) 
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./Fxc7/loli2.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, loli2, image, {quoted: mek, caption: '\`\`\`LOLI\`\`\`'})
				await limitAdd(sender) 
				break
			    case 'nsfwblowjob':
				    try {
            ...
            client.updatePresence(from, Presence.composing) 
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./Fxc7/songsanime.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, songsanime, image, {quoted: mek, caption: '\`\`\`songsanime\`\`\`'})
				await limitAdd(sender) 
				break
					case 'yuri':
            client.updatePresence(from, Presence.composing) 
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./Fxc7/yurichan.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, yurichan, image, {quoted: mek, caption: '\`\`\`yurichan\`\`\`'})
				await limitAdd(sender) 
				break
            case 'milfsan'
            client.updatePresence(from, Presence.composing) 
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./Fxc7/milfsan.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, milfsan, image, {quoted: mek, caption: '\`\`\`milfsan\`\`\`'})
				await limitAdd(sender) 
				break
					case 'milf':  
     client.updatePresence(from, Presence.composing) 
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./Fxc7/milf.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, milf, image, {quoted: mek, caption: '\`\`\`milf\`\`\`'})
				await limitAdd(sender) 
				break
            case 'cosplay': 
     client.updatePresence(from, Presence.composing) 
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./Fxc7/cosplay.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, cosplay, image, {quoted: mek, caption: '\`\`\`cosplay\`\`\`'})
				await limitAdd(sender) 
				break
				case 'nsfwtrap':
				    try {
            ...
            client.updatePresence(from, Presence.composing) 
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./Fxc7/doujin.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 client.sendMessage(from, doujin, image, {quoted: mek, caption: '\`\`\`doujin\`\`\`'})
				await limitAdd(sender) 
				break
				case 'hentai': 
               try {
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[SYMPHONYBOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
